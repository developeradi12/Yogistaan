...
export default function TripCard({ item }: { item: any }) {
  return (
    <div className="relative h-[420px] rounded-3xl overflow-hidden border border-[#C8A96A]/20 shadow-md hover:shadow-xl transition-all duration-300 group flex flex-col">
      ...
      {/* Top badges */}
      <div className="absolute top-4 left-4 bg-[#C4622A] text-white text-xs font-bold px-3 py-1 rounded-full uppercase tracking-widest shadow">
        {item.duration}
      </div>
      ...
      <div className="absolute bottom-0 left-0 right-0 p-4">
        <div className="bg-white/95 backdrop-blur-md rounded-2xl p-4 shadow-xl flex flex-col h-[180px]">
          ...
          <Link href={`/tripDetail/${item.slug}`}>
            <Button className="w-[40%] rounded-full bg-[#1C3A2F] hover:bg-[#C4622A] text-white text-xs font-semibold h-8">
              Book Now →
            </Button>
          </Link>
